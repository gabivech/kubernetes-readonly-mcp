...
public final class KubernetesMcpApplication {
    ...
    private static SyncToolSpecification tool(String name, String description, Map<String, Object> schema, ToolHandler handler) {
        return SyncToolSpecification.builder().tool(McpSchema.Tool.builder(name, schema).description(description).build()).callHandler((exchange, request) -> {
            try { return handler.handle(exchange, request); }
                ...
                return McpSchema.CallToolResult.builder().isError(true).content(List.of(McpSchema.TextContent.builder("Erro: " + exception.getMessage()).build())).build();
            }
        }).build();
    }
    ...
    private static String stringArgument(McpSchema.CallToolRequest request, String name) { return String.valueOf(request.arguments().get(name)); }
    ...
    private static int integerArgument(McpSchema.CallToolRequest request, String name, int fallback) { Object value = request.arguments().get(name); return value instanceof Number number ? number.intValue() : fallback; }
    ...
    @FunctionalInterface private interface ToolHandler { McpSchema.CallToolResult handle(Object exchange, McpSchema.CallToolRequest request) throws Exception; }
}
